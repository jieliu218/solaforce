...
function getResourceTypeString(resourceType) {
    ...
    if (!resourceType.isIframe && !resourceType.isForm && !resourceType.isScript && !resourceType.isEventSource && !resourceType.isHtmlImport) return null;

    return [resourceType.isIframe ? 'i' : '', resourceType.isForm ? 'f' : '', resourceType.isScript ? 's' : '', resourceType.isEventSource ? 'e' : '', resourceType.isHtmlImport ? 'h' : ''].join('');
}
...
function prepareUrl(url) {
    ...
    return url.replace(/^(https?:)?\/+(\/\/.*$)/i, '$1$2').replace(/^file:\/*([A-Za-z]):/, 'file:///$1:');
}
